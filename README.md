...
- 模型库显示 Go 套餐官方参考价，包含输入、输出、缓存读/写、折扣和免费模型；实际扣费仍以 Command Code Studio 为准
...
模型库同时展示 Command Code 官方 Go 套餐参考价。页面中的折扣、免费活动和 DeepSeek 高峰/非高峰价会以快照时间标注；这里只用于选型比较，实际账单以 Command Code Studio 的 Usage 页面为准。
